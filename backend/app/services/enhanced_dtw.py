...
class DTWEnsemble:
    ...
    def __init__(
        ...
        if algorithm_weights is None:
            # CRITICAL FIX v2: HEAVILY emphasize direction-based matching
            # to distinguish gestures with same hand shape but different movements:
            # - Swipe left vs swipe right (same hand shape, opposite direction)
            # - Swipe up vs swipe down (same hand shape, opposite direction)
            # - Circle clockwise vs counter-clockwise (same shape, different trajectory)
            self.algorithm_weights = {
                'standard': 0.20,        # Reduced: basic hand shape/position
                'direction': 0.50,       # INCREASED: movement direction is CRITICAL
                'multi_feature': 0.30    # Moderate: combined features with velocity
            }
            logger.info("✅ DIRECTION-FOCUSED DTW weights: standard=20%, direction=50%, multi_feature=30%")
        else:
    ...
    def match(
        ...
        if self.algorithm_weights.get('direction', 0) > 0:
            # CRITICAL FIX v2: HEAVILY emphasize direction over position
            # alpha=0.75 means 75% weight on direction, 25% on position
            # This is crucial for distinguishing gestures with different movement directions
            dir_dist = self.dtw.direction_similarity_dtw(seq1, seq2, alpha=0.75)
            dir_sim = self.dtw.calculate_similarity(dir_dist)
            results['direction'] = dir_sim

        # 3. Multi-Feature DTW
        if self.algorithm_weights.get('multi_feature', 0) > 0:
            # CRITICAL FIX v2: Emphasize velocity (movement) over position (hand shape)
            mf_dist, _ = self.dtw.multi_feature_dtw(
                seq1, seq2,
                weights={
                    'pos': 0.35,  # Reduced: position/hand shape less important
                    'vel': 0.50,  # INCREASED: velocity is critical for movement direction
                    'acc': 0.15   # Moderate: acceleration for dynamics
                }
            )
